...
> **Version 1.1.3** · Funktional vollständig und automatisiert geprüft, inklusive Layout von 320 px bis Desktop. Drei manuelle Abnahmepunkte sind noch offen (Cross-Browser, Tastaturbedienung, Zielumgebungs-Proxy) — siehe [docs/requirements.md](docs/requirements.md#9-abnahmekriterien).
...
```bash
docker build -t mmo-vault:1.1.3 .
...
```bash
docker save mmo-vault:1.1.3 | gzip | ssh server 'gunzip | docker load'
```
